...
//        button.setPrefSize(50 * StandardParameter.SCALE, 15 * StandardParameter.SCALE);
    ...
    public void update(WordBoard wordBoard) throws SQLException {
        ...
        for (int j = 0; j < contronymList.size() && i < contronymFlowPane.getChildren().size(); j++) {
            ...
            if (!SQLDictionary.isExisted(word)) {
                button.setOnMouseMoved(e -> {
                    Canvas.dialog.getLayout().setOpacity(1);
                    Canvas.dialog.getLayout().toFront();
                    Canvas.dialog.setText("Not in sql.dictionary");
                    Canvas.dialog.getLayout().setOpacity(1);
                    Canvas.dialog.getLayout().setLayoutX(e.getSceneX());
                    Canvas.dialog.getLayout().setLayoutY(e.getSceneY());

                });
                button.setOnMouseExited(e -> {
                    Canvas.dialog.getLayout().setOpacity(0);
                });
            } else {
